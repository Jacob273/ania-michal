...
export class VegetableGameService {
  ...
  private loadStats(player: 'ANIA' | 'MICHAL'): VegetableStats {
  ...
  private saveStats(player: 'ANIA' | 'MICHAL', stats: VegetableStats): void {
    const key = player === 'ANIA' ? this.STORAGE_KEY_ANIA : this.STORAGE_KEY_MICHAL;
    localStorage.setItem(key, JSON.stringify(stats));
  }

  getVegetableStats(player: 'ANIA' | 'MICHAL'): VegetableStats {
    return this.loadStats(player);
  }

  incrementVegetablesCompleted(player: 'ANIA' | 'MICHAL'): void {
